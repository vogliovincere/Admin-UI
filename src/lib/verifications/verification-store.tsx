...
export interface VerificationContextValue {
  ...
  // EDD collection-link lifecycle events (append timeline entries only).
  recordLinkEvent: (
    sessionId: string,
    personId: string | null,
    kind: "generated" | "cancelled" | "regenerated"
  ) => void;
      ...
      const { action, detail } =
        kind === "generated"
          ? {
              ...
              detail: `EDD collection link generated by ${ADMIN_ACTOR_NAME}`,
            }
          : kind === "cancelled"
            ? {
                action: "Link cancelled",
                detail: `EDD collection link cancelled by ${ADMIN_ACTOR_NAME}`,
              }
            : {
                action: "New link generated",
                detail: `New EDD collection link generated by ${ADMIN_ACTOR_NAME}`,
              };
      update((curr) => {
        const session = curr.find((s) => s.id === sessionId);
        if (!session) return curr;
        ...
        // Always an individual entry; entity sessions additionally get a
        // business-group entry.
        const entries: SessionTimelineEntry[] = [
          { ...base, ...subjectFor(session, personId) },
        ];
        if (session.pathType === "entity") {
          entries.push({
            ...base,
            id: timelineId(),
            ...subjectFor(session, null),
          });
        }
        return curr.map((s) =>
          s.id === sessionId
